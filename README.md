...
- **Frontend:** HTML/CSS/JS puro, sin build ni framework — se despliega directo a GitHub Pages:
  ...
  - `js/00-config.js` … `js/16-fondo-emergencia.js`: el código, por tema. Son scripts clásicos que comparten el ámbito global y se cargan en el orden de `index.html`; una función usada **al cargar** debe estar en el mismo archivo o en uno anterior (`tests/arranque-sin-errores.test.js` lo vigila)
