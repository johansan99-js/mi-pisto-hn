...
function renderDashboardGoals(){const container=document.getElementById('dashboard-goals');if(!container)return;container.innerHTML=state.goals.slice(0,3).map(g=>{const pct=Math.min(100,(g.actual/g.objetivo)*100);return `<div class="goal-mini"><div class="goal-mini-header"><span>${esc(g.nombre)}</span><span>${pct.toFixed(0)}%</span></div><div class="goal-mini-bar"><div class="goal-mini-progress" style="width:${pct}%"></div></div></div>`}).join('')+(state.goals.length>3?`<a href="#" class="goal-mini-mas" style="display:block;text-align:center;font-size:12px;color:var(--green);padding:6px" onclick="event.preventDefault();switchView('metas')">Ver las ${state.goals.length} metas →</a>`:'')}
